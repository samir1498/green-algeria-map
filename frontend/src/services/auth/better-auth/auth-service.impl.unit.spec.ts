...
vi.mock('@/services/error/error-handler', () => ({
  normalizeAuthError: (e: any) => ({ message: e.message, code: e.code ?? 'UNKNOWN', category: 'auth' }),
}))
...
describe('betterAuthService', () => {
  describe('signIn', () => {
    ...
    it('returns error when sign in fails', async () => {
      mockSignInEmail.mockResolvedValueOnce({ data: null, error: { message: 'Invalid credentials', code: 'INVALID_CREDENTIALS' } })
      const result = await betterAuthService.signIn({ email: 'test@example.com', password: 'wrong' })
  ...
  describe('signUp', () => {
    it('returns user on success', async () => {
      ...
      const result = await betterAuthService.signUp({ name: 'New User', email: 'new@example.com', password: 'pass' })
    ...
    it('returns error on failure', async () => {
      ...
      const result = await betterAuthService.signUp({ name: 'New User', email: 'taken@example.com', password: 'pass' })
