...
public record DamageReportResponse(
    ...
    public static DamageReportResponse from(DamageReport report) {
        return new DamageReportResponse(
            report.getId(), report.getZoneId(), report.getType(), report.getSeverity(),
            report.getStatus(), report.getLat(), report.getLng(), report.getDescription(),
            report.getReportedBy(), report.getReportedAt(), report.getUpdatedAt()
        );
    }
}
