...
describe('Zone', () => {
  it('creates with defaults', () => {
    const zone = Zone.create({
      ...
      type: 'cleanup',
      status: 'planned',
      coordinates: new Coordinates(36.7, 3.0),
      description: '',
    });
  ...
  it('exports error classes', async () => {
    const { InvalidZoneTypeError, InvalidZoneStatusError } =
      await import('./zone-errors');
    expect(() => {
      throw new InvalidZoneTypeError('bad');
    }).toThrow('Invalid zone type: bad');
    expect(() => {
      throw new InvalidZoneStatusError('bad');
    }).toThrow('Invalid zone status: bad');
  });
});
