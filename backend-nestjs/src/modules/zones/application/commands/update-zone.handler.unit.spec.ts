...
const mockRepo = { findAll: vi.fn(), findById: mockFindById, save: mockSave, remove: mockRemove };
...
describe("UpdateZoneHandler", () => {
  it("updates all fields and saves", async () => {
    const zone = { id: "z-1", name: "Old", rename: vi.fn(), updateType: vi.fn(), reposition: vi.fn(), updateTargetCount: vi.fn(), updateProgress: vi.fn(), changeStatus: vi.fn(), updateDescription: vi.fn(), updateOrganizerContact: vi.fn(), updateTreeSpecies: vi.fn(), coordinates: { lat: 1, lng: 2 } };
    mockFindById.mockResolvedValue(zone);
    mockSave.mockResolvedValue(zone);

    const cmd = new UpdateZoneCommand("z-1", "New", "trash", "in-progress", 10, 20, 50, 25, "desc", "c@e.com", "pine");
  ...
  it("handles partial update with some undefined fields", async () => {
    const zone = { id: "z-1", name: "X", rename: vi.fn(), updateType: vi.fn(), reposition: vi.fn(), updateTargetCount: vi.fn(), updateProgress: vi.fn(), changeStatus: vi.fn(), updateDescription: vi.fn(), updateOrganizerContact: vi.fn(), updateTreeSpecies: vi.fn(), coordinates: { lat: 1, lng: 2 } };
    mockFindById.mockResolvedValue(zone);

    // only name and targetCount set — others should be undefined (no calls)
    await updateHandler.execute({ id: "z-1", name: "Renamed", targetCount: 200 } as any);
  ...
  it("throws NotFoundException when zone missing", async () => {
    ...
    await expect(updateHandler.execute({ id: "missing" } as any)).rejects.toThrow(NotFoundException);
  });
});
...
describe("DeleteZoneHandler", () => {
  ...
  it("throws NotFoundException when zone missing", async () => {
    mockFindById.mockResolvedValue(null);
    await expect(deleteHandler.execute({ id: "missing" } as any)).rejects.toThrow(NotFoundException);
  });
});
