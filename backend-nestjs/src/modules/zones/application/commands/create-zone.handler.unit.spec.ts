...
const handler = new CreateZoneHandler(mockRepository as any, mockEventBus as any);

beforeEach(() => vi.resetAllMocks());

describe("CreateZoneHandler", () => {
  const command = new CreateZoneCommand("Park", "planting", 36.75, 3.05, "planned", 100, 0, "desc", "c@e.com", "cedar");

  it("creates zone and publishes event", async () => {
    mockRepository.save.mockResolvedValue(
      Object.assign(Zone.create({ name: "Park", type: "planting", status: "planned", coordinates: new Coordinates(36.75, 3.05), description: "" }), { id: "z-1" }),
    );
    ...
    expect(saved).toMatchObject({ name: "Park", type: "planting", targetCount: 100, organizerContact: "c@e.com" });
